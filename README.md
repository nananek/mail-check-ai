...
- 📄 **多様なファイル対応**: PDF、Word (DOCX)、Excel (XLSX)、CSV、テキストファイルの自動解析
...
メールの添付ファイルから自動的にテキストを抽出し、GPTに送信します：

| 形式 | 拡張子 | 対応状況 | 備考 |
|------|--------|---------|------|
| PDF | `.pdf` | ✅ 完全対応 | PyMuPDFでテキスト抽出 + OCR対応 |
| PDF (画像のみ) | `.pdf` | ✅ OCR対応 | Tesseract OCR（日本語+英語）で自動認識 |
| Word | `.docx` | ✅ 完全対応 | python-docxで本文・テーブル抽出 |
| Word (旧) | `.doc` | ⚠️ 非対応 | .docx形式に変換が必要 |
| Excel | `.xlsx` | ✅ 完全対応 | openpyxlで全シート・全行を抽出 |
| Excel (旧) | `.xls` | ⚠️ 非対応 | .xlsx形式に変換が必要 |
| CSV | `.csv` | ✅ 完全対応 | 文字コード自動判定 (UTF-8, Shift-JIS等) |
| テキスト | `.txt`, `.log`, `.md` | ✅ 完全対応 | 文字コード自動判定 |
| 構造化文書 | `.json`, `.xml`, `.html` | ✅ 完全対応 | 文字コード自動判定 |

**PDF特有の機能：**
- 📄 テキスト埋め込みPDF: 通常のテキスト抽出
- 🖼️ スキャン画像PDF: Tesseract OCRで自動テキスト化（日本語+英語対応）
- 🔄 ページ向き自動検出: 回転が必要なページを自動補正
- 🎯 ハイブリッド処理: テキストが少ない場合のみOCRを実行（性能最適化）

**制限事項：**
- 1ファイルあたり最大10,000文字まで（超過分は省略）
- 表形式データ（Excel/CSV）は最大500行まで
- GPTへの送信は添付ファイル全体で最大8,000文字まで

**エラーハンドリング：**
- ファイル解析に失敗した場合でもメール処理は継続
- 未対応形式の場合は「[未対応のファイル形式]」と記録
- エラー内容は返信案に自動的に含まれます
